...
unsigned get_largest_element(unsigned * arr, size_t n) {
  size_t largest_element = 0;
  for (size_t i = 1; i < n; i++) {
    if (arr[i] > arr[largest_element]){
      largest_element = i;
    }
  }
  return arr[largest_element];
}

size_t get_match_index(unsigned * match_counts, size_t n, unsigned n_of_a_kind){
  for (size_t i = 0; i < n; i++) {
    if (match_counts[i] == n_of_a_kind){
      return i;
    }
  }
  return 0;
...
hand_eval_t build_hand_from_match(deck_t * hand,
  ...
  for (size_t i = idx; i < (size_t) n + idx; i++) {
    
    card_t **temp_hands = hand -> cards;
    card_t *temp_hand = temp_hands[i];
    ans.cards[ansFirst] = temp_hand;
  ...
  size_t ansRemainder = 0;

  for (size_t j = ansFirst; j < 5; j++) {
    if (ansRemainder < idx || ansRemainder >= idx + (size_t) n) {
      card_t **temp_hands = hand -> cards;
      card_t * temp_hand = temp_hands[ansRemainder];
      ans.cards[j] = temp_hand;
      
      ansRemainder++;
    }
    else {
      ansRemainder += (size_t)n;
      card_t **temp_hands = hand -> cards;
      card_t * temp_hand = temp_hands[ansRemainder];
      ans.cards[j] = temp_hand;
      ansRemainder++;
...
int compare_hands(deck_t * hand1, deck_t * hand2) {
  ...
  hand_eval_t evaluate_hand1 = evaluate_hand(hand1);
  hand_eval_t evaluate_hand2 = evaluate_hand(hand2);
  
  if (evaluate_hand1.ranking < evaluate_hand2.ranking) {
    return 1;
  }
  else if (evaluate_hand2.ranking < evaluate_hand1.ranking) {
    return -1;
  }
  else {
    for (size_t i = 0; i < 5; i++) {
      if (evaluate_hand1.cards[i] -> value > evaluate_hand2.cards[i] -> value){
	return 1;
      }
      else if (evaluate_hand2.cards[i] -> value > evaluate_hand1.cards[i] -> value) {
	return -1;
      }
    }
  }
...
unsigned * get_match_counts(deck_t * hand) {
  ...
  /*     if (currCard.value == hand->cards[j]->value) { */
  /* 	count++; */
  /*     } */
  /*   } */
